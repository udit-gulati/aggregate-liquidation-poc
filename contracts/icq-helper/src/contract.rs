...
use crate::msg::{ExecuteMsg, InstantiateMsg, MigrateMsg, QueryMsg, NewDelegateResponse};
use crate::state::{ DELEGATE_AMOUNT, DELEGATION_FIELD, HEIGHT_FIELD, COSMOS_SDK_STAKING_MSG_URL};
use neutron_sdk::bindings::msg::NeutronMsg;
use neutron_sdk::bindings::query::{NeutronQuery, QueryRegisteredQueryResponse};
use neutron_sdk::interchain_queries::v045::queries::{query_delegations};
...
const CONTRACT_NAME: &str = concat!("crates.io:neutron-", env!("CARGO_PKG_NAME"));
...
pub fn execute(
    ...
    match msg {
        ExecuteMsg::RegisterVotingPowerQuery {
            connection_id,
            addr,
            valaddrs,
            update_period,
        } => register_voting_power_query(connection_id, addr,valaddrs, update_period),
...
pub fn register_voting_power_query(
    ...
    let msg = new_register_delegator_delegations_query_msg(connection_id, addr,valaddrs,update_period)?;
...
pub fn register_stake_query_msg(
    ...
    let msg = new_register_stake_query_msg(connection_id,update_period,min_height)?;
...
pub fn sudo_kv_query_result(
    ...
    let delegations = query_delegations(deps.as_ref(), env, query_id)?;
